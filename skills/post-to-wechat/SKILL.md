...
test -f .yunhe-skills/post-to-wechat/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.yunhe-skills/post-to-wechat/EXTEND.md" && echo "user"
```

┌────────────────────────────────────────────────────────┬───────────────────┐
│                          Path                          │     Location      │
├────────────────────────────────────────────────────────┼───────────────────┤
│ .yunhe-skills/post-to-wechat/EXTEND.md           │ Project directory │
├────────────────────────────────────────────────────────┼───────────────────┤
│ $HOME/.yunhe-skills/post-to-wechat/EXTEND.md     │ User home         │
└────────────────────────────────────────────────────────┴───────────────────┘
...
| API credentials | Follow guided setup in Step 2, or manually set in `.yunhe-skills/.env` |
...
```bash
# Check project-level
test -f .yunhe-skills/.env && grep -q "WECHAT_APP_ID" .yunhe-skills/.env && echo "project"

# Check user-level
test -f "$HOME/.yunhe-skills/.env" && grep -q "WECHAT_APP_ID" "$HOME/.yunhe-skills/.env" && echo "user"
```
...
Where to save?
A) Project-level: .yunhe-skills/.env (this project only)
B) User-level: ~/.yunhe-skills/.env (all projects)
```
...
- Guided setup in Step 2, or manually set in `.yunhe-skills/.env`
